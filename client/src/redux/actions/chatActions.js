...
export const updateTypingUsers = (action, displayName) => dispatch => {
  dispatch({
    type: action === 'addUser' ? ADD_TYPING_USER : REMOVE_TYPING_USER,
    payload: displayName,
  });
};
...
export const handleFileUpload = file => dispatch => {
  ...
  const allowedFileTypes = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'];
  // Check File Type
  const fileType = file.type.split('/')[1];
  if (!allowedFileTypes.includes(fileType))
    return dispatch(setToast('הקובץ שנבחר לא נתמך במערכת', 'error'));
  ...
  if (fileSizeInKB > maxFileSize)
    return dispatch(
      setToast(
        `הקובץ שנבחר גדול מדי, הגודל המירבי הניתן להעלאה הינו ${Math.ceil(
          maxFileSize / 1024
        )}MB`,
        'error'
      )
  ...
  fetchUserSuggestionsTimeout = setTimeout(() => {
    if (displayName.trim().length) {
      dispatch({
        ...
        payload: axios.get(
          `${API_URL}/users?displayName=${displayName}&limit=5`
        ),
      });
    }
  }, 400);
